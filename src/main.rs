//! `git-shell` is a simplified replacement for `git/git-shell`.
//!
//! It can only be called with `-c cmd` option, allows UTF-8 input only and will give better error
//! messages than the git version. It doesn't honor the `GIT_EXEC_PATH` runtime environment
//! variable, git executable path is configured at compile time using `GIT_EXECUTABLE` environment
//! variable or defaults to `/usr/bin/git`.
...
const GIT_EXECUTABLE: &str = match option_env!("GIT_EXECUTABLE") {
    Some(git) => git,
    None => "/usr/bin/git",
};
...
fn git() -> Result<Command> {
    let git = Path::new(GIT_EXECUTABLE);
        ...
        git.is_absolute(),
        "GIT_EXECUTABLE={:?} was not an absolute path, please recompile the binary",
        &git
    );
...
fn standard_commands(cmd: &str) -> Result<()> {
    let (cmd, arg) = cmd.split_once(" ").context("missing command argument")?;

    let arg =
        git_shell_dequote(arg).with_context(|| format!("malformed command argument: {}", arg))?;

    Err(git()?.arg(cmd).arg(arg).exec()).context("failed to exec git")
}
...
fn git_shell_dequote(input: &str) -> Result<String> {
    let mut input = input
        .strip_prefix('\'')
        .context("missing ' at the beginning")?
        .strip_suffix('\'')
        .context("missing ' at the end")?
        .as_bytes();
